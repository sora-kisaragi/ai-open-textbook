...
def test_sqlite_projects_curriculum_and_coverage(tmp_path: Path) -> None:
    ...
    assert counts[:5] == (44, 32, 96, 96, 302)
    with sqlite3.connect(output) as connection:
        ...
        ).fetchall() == [("not_started", 94), ("partial", 2)]
        assert connection.execute(
...
def test_invalid_source_does_not_replace_existing_index(tmp_path: Path) -> None:
    ...
    lesson["updated_at"] = "not-a-date"
    lesson_path.write_text(json.dumps(lesson, ensure_ascii=False) + "\n", encoding="utf-8")
